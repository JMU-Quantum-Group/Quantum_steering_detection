...
class ML_PIC(object):
    def __init__(self, N, n_points, rho, partition_list, r, lr=0.01):
        ...
        # self.beta = torch.flatten(self.white_noise - torch.tensor(rho, dtype=torch.complex128))
    ...
    def train(self, epoch):
        ...
        optimizer = torch.optim.AdamW(opti_list, lr=self.lr)
        ...
        for epoch in range(epoch):
            ...
            for point_index in range(self.n_points):
                ...
                for partition_index in range(len(self.partition_list)):
                    ...
                    current_target = weights_normalized[point_index * len(self.partition_list) + partition_index] * current_target
    ...
    def sdp(self):
        ...
        for point_index in range(self.n_points):
            for partition_index in range(len(self.partition_list)):
                ...
                for exchange_pair in self.exchange_list[partition_index]:
                    current_rho = self.exchange_matrix_np[exchange_pair[0]][exchange_pair[1] - exchange_pair[0] - 1] * current_rho * self.exchange_matrix_np[exchange_pair[0]][exchange_pair[1] - exchange_pair[0] - 1]
